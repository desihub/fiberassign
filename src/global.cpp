        ...
        if (g!=-1&&M[g].t_priority!=9800 && M[g].t_priority!=9900){
            ...
            if(M[g].nobs_remain=F.goal[G[g].id]){//first obs
                if(F.goalpost[G[g].id]==1){//if only one obs needed
                    ...
                    M[g].t_priority=F.priopost[G[g].id];
                    if(G[g].id==2&&g%100000==0){
                        printf("first time LRG more obs needed  nobs_remain %d nobs_done %d id %d goal d% goalpost   %dn",M[g].nobs_remain,M[g].nobs_done,G[d].id,F.goal[G[g].id],F.goalpost[G[g].id]);
                    }
                }
            }
