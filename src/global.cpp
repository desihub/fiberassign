    ...
    for(int j=0; j<F.Nplate; j++) {
        for(int k=0; k<F.Nfiber; k++) {
            for(int m=0; m<P[j].av_gals[k].size(); m++) {
                //i is the id of the mth galaxy available to tile j and fiber k
            ...
            for (int i=0; i<av_g.size(); i++) {
                ...
                if (g!=-1 && g!=no_g && !M[g].SS && !M[g].SF) {
                    ...
                    if (ok_assign_g_to_jk(g,j,k,P,M,pp,F,A) ) {
                        ...
                        //all tile-fibers that observe g in tiles from begin to end
                        Plist tfs = A.chosen_tfs(g,F,A.suborder[jused_begin]);
...
void new_replace( int j, int p, MTL& M, Plates& P, const FP& pp, const Feat& F, Assignment& A) {
    ...
    for(int c=0;P[j].SS_in_petal[p]<F.MaxSS && c<M.priority_list.size();++c ){
		...
		std::vector<int> sorted_could_be_replaced;
                for(int i=0;i<tfs.size() && done==0;++i){
                    ...
                    if(tfs[i].f==j&&pp[tfs[i].s].spectrom==p){
                        ...
                        //we know g can be reached by this petal of plate j and fiber k
                        int k=tfs[i].s;

                        //what is now at (j,k)  g_old can't be -1 or we would have used it already in assign_sf
                        int g_old=A.TF[j][k];
    ...
    for(int c=0;P[j].SF_in_petal[p]<F.MaxSF && c<M.priority_list.size();++c ){
                ...
                if (prio>pbest || (prio==pbest && m>mbest) || (prio==pbest && m==mbest && subprio>subpbest)) {
                    if (A.is_assigned_jg(j,g,M,F)==-1 && ok_assign_g_to_jk(g,j,k,P,M,pp,F,A)&&ok_for_limit_SS_SF(g,j,k,M,P,pp,F)){
                        ...
                        for (int i=0; i<A.GL[g].size(); i++) {
                            //GL[g].size() is number of tf that could observe g
                            int jp = A.GL[g][i].f;
                            int kp = A.GL[g][i].s;
                            if (j<jp && jpb<jp) {
            ...
            if (Done[jused][k]==0) {
                ...
                if (g!=-1 && !M[g].SS && !M[g].SF) {
                    ...
                    for (int i=0; i<av_tfs.size(); i++) {
                        ...
                        int unused = A.unused[jp][pp[kp].spectrom];
                        if(A.inv_order[jp]!=-1) {
                            //necessary because underdense targets may leave some plates unused
