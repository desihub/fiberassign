...
void element::print () const {
    ...
    fl();
    if (!O.isnull() || (rad != 0) ) {
        printf(" - center (%f,%f) rad %f", O.f, O.s, rad);
        fl();
    }
    printf(" sizesegs= %lu ", segs.size() );
    fl();
    if (0 < segs.size() ) {
        debl(" - segs : ");
        for (int i = 0; i < segs.size(); i++) {
...
polygon create_fh () {
    ...
    // el.add(-2.235,0.990); el.add(0.387,0.990); el.add(0.967,0.410);
    // el.add(0.967,-0.410); el.add(0.387,-0.990); el.add(-1.844,-0.990);
    // el.add(-1.981,-1.757); el.add(-2.688,-2.015); el.add(-2.944,-1.922);
...
Dlist angles (const dpair & A, const PosP & posp) {
    ...
    Dlist ang;
    ang.push_back(cos(theta) );
    ang.push_back(sin(theta) );
    ang.push_back(cos(phi) );
    ang.push_back(sin(phi) );
    return ang;
}
