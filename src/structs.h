...
class plate {
    public:
        ...
        List av_gals_plate (const Feat & F, const MTL & M, const
                            FP & pp) const;
};
...
class Assignment {
    public:
        ...
        void unassign (int j, int k, int g, MTL & M, Plates & P, const
                       FP & pp);

        int find_collision (int j, int k, int g, const FP & pp, const MTL & M,
                            const Plates & P, const Feat & F, int col =
                                -1) const;

        bool find_collision (int j, int k, int kn, int g, int gn, const
                             FP & pp, const MTL & M, const Plates & P, const
                             Feat & F, int col =
                                 -1) const;

        int is_collision (int j, int k, const FP & pp, const MTL & M, const
                          Plates & P, const Feat & F) const;

        // Verif mappings are right
        void verif (const Plates & P, const MTL & M, const FP & pp, const
                    Feat & F) const;
        ...
        List fibs_of_kind (int kind, int j, int pet, const MTL & M, const
                           FP & pp, const Feat & F) const;

        // not used
        // Sort this list of fibers by decreasing density
        List sort_fibs_dens (int j, const List & fibs, const MTL & M, const
                             Plates & P, const FP & pp, const Feat & F) const;
        ...
        float colrate (const FP & pp, const MTL & M, const Plates & P, const
                       Feat & F, int j = -1) const;

        // Know the number of remaining observations of g when the program is
        // at the tile j, for pyplotTile
        int nobs_time (int g, int j, const Gals & Secret, const MTL & M, const
                       Feat & F) const;
