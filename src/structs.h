...
class FP  : public std::vector<struct fpos>{
    public:
        // Inv map of spectrom, fibers_of_sp[sp] are fibers of spectrom sp, redundant
        Table fibers_of_sp;
};
...
//note that this is how we read it in! not long
  ...
  char brickname [9];
  Plist av_tfs;
  // 16bit mask indicating under what conditions this target can be observed.
  uint16_t obsconditions;
};
...
class Assignment {
    ...
    // TF for tile fiber, #tiles X #fibers TF[j][k] is the chosen galaxy, -1 if not yet chosen
    Table TF;

    // Order of tiles we want to assign, only 1-n in simple increasing order for the moment
    List order;
    ...
    // Next plate in the order, i.e suborder(next_plate) is actually next plate
    int next_plate;

    // Redundant information (optimizes computation time)

    // GL for galaxy - list : #galaxies X (variable) #chosen TF: gives chosen tf's for galaxy g
    Ptable GL;

    // Cube[j][sp][id] : number of fibers of spectrometer sp and plate j that have the kind id
    Cube kinds;
    ...
    // List of nobs, redundant but optimizes, apparent goal, i.e. goal of category of this type, gets updated
    //List nobsv_tmp;
    ...
    void verif(const Plates& P, const MTL& M, const FP& pp, const Feat& F) const;
    ...
    // Counts how many more times object should be observed. If tmp=true, return maximum for this kind (temporary information)
    ...
    // Number of fibers assigned to the kind "kind" on the petal of (j,k). If pet=true, we don't take k but the petal p directly instead
    int nkind(int j, int k, int kind, const MTL& M, const Plates& P, const FP& pp, const Feat& F, bool pet=false) const;

    // Sublist of fibers assigned to a galaxy of type kind for (j,p)
    List fibs_of_kind(int kind, int j, int pet, const MTL& M, const FP& pp, const Feat& F) const;

    //not used
    // Sort this list of fibers by decreasing density
    List sort_fibs_dens(int j, const List& fibs, const MTL& M, const Plates& P, const FP& pp, const Feat& F) const;

    // Sublist of unassigned fibers for (j,p)
    List fibs_unassigned(int j, int pet, const MTL& M, const FP& pp, const Feat& F) const;
    ...
    float colrate(const FP& pp, const MTL& M, const Plates& P, const Feat& F, int j=-1) const;

    // Know the number of remaining observations of g when the program is at the tile j, for pyplotTile
    int nobs_time(int g, int j, const Gals& Secret,const MTL& M, const Feat& F) const;
...
// collisions from  looking at galaxy G1 with fiber positioner centered at 01 and etc calculated in mm on plate
...
// weighted (and only with remaining observation according to the moment in the survey), and doesn't take into account other kinds than QSO LRG ELG not used
int num_av_gals(int j, int k, const MTL& M, const Plates& P, const Feat& F, const Assignment& A);
